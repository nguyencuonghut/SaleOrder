          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Tất cả phân loại</h3>
              ...
              <div class="card-body table-responsive p-0">
                <a style="margin: 10px;" href="{{route('categories.create')}}" wire:navigate><button class="btn btn-success btn-sm"><i class="fa fa-plus"></i></button></a>
                <table class="table table-hover text-nowrap">
                  <thead>
                    <tr>
                      ...
                      <th wire:click.prevent="sortBy('description')" ><a role="button" href="#" style="color:#212529">Mô tả</a>
                        @if($sortField == 'description')
                          <i class="fa {{ $sortAsc == true ? 'fa-sort-up' : 'fa-sort-down' }}" style=" {{ $sortField == 'description' ? '' : 'color:#cccccc'}} "></i>
                        @else
                          <i class="fa fa-sort" style="color:#cccccc"></i>
                        @endif
                      </th>
                  ...
                  <tbody>
                    ...
                    @foreach ($categories as $category)
                        <tr>
                            <td>{{++$sl}}</td>
                            <td>
                                @if ($editCategoryIndex === $category->id)
                            ...
                            <td>
                                ...
                                @elseif ($deletedCategoryIndex === $category->id)
                                    <button type="button" wire:click.prevent="destroy" class="btn btn-outline-success btn-sm"><i class="fa fa-check-square"></i></button>
                                    <button type="button" wire:click.prevent="cancel" class="btn btn-outline-danger btn-sm"><i class="fa fa-times-circle"></i></button>
                                @else
                                    <button type="button" wire:click.prevent="edit({{$category->id}})" class="btn btn-outline-warning btn-sm"><i class="fa fa-edit"></i></button>
                                    <button type="button" wire:click.prevent="confirmDestroy({{$category->id}})" class="btn btn-outline-danger btn-sm"><i class="fa fa-trash"></i></button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
